...
void Animation::Update(float DeltaTime)
{
	...
	if (m_CurClip->m_Option == AO_LOOP || m_CurClip->m_Option == AO_ONCE_DESTROY || m_CurClip->m_Option == AO_ONCE_RETURN || m_CurClip->m_Option == AO_BOUNCE_LOOP_LEMUIRAN || m_CurClip->m_Option == AO_LOOP_WISP || m_CurClip->m_Option == AO_LOOP_BOSS_BOUNCE)
	{
	...
	else if (m_CurClip->m_Option == AO_REVERS_LOOP || m_CurClip->m_Option == AO_REVERS_BOUNCE_LOOP || m_CurClip->m_Option == AO_REVERS_BOUNCE_LOOP_LEMUIRAN || m_CurClip->m_Option == AO_REVERS_WISP || m_CurClip->m_Option == AO_REVERS_LOOP_BOSS_BOUNCE)
	{
		while (m_CurClip->m_AnimationTime >= CompleatTime)
		{
			m_CurClip->m_AnimationTime -= CompleatTime;
