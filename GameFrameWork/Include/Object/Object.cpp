...
void Object::Render(HDC hDC, float DeltaTime)
{
	if (m_Texture != NULL)
	{
		...
		else
		{
			...
			else if ((m_Animation != NULL && m_Animation->GetOption() == AO_REVERS_LOOP_BOSS_BOUNCE ) || (m_Animation != NULL && m_Animation->GetOption() == AO_LOOP_BOSS_BOUNCE))
				TransparentBlt(hDC, (int)LeftTop.x - (int)tSize.GetHalfX() / (int)2.0f, (int)LeftTop.y , (int)tSize.x, (int)tSize.y, m_Texture->GetMemDC(), FrameX, FrameY, (int)tSize.x, (int)tSize.y, (UINT)m_ColorKey);
			else
				TransparentBlt(hDC, (int)LeftTop.x, (int)LeftTop.y, (int)tSize.x, (int)tSize.y, m_Texture->GetMemDC(), FrameX, FrameY, (int)tSize.x, (int)tSize.y, (UINT)m_ColorKey);
		}
	}
